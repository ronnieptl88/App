...
export default {
    ...
    MONEY_REQUEST: {
        ROOT: createRouteWithGetter({         ':iouType/:reportID?'                                 : (iouType: string, reportID = '') => `${iouType}/${reportID}`}),
        TABS: {
            MANUAL: createRouteWithGetter({   ':iouType/manual/new/:reportID?'                      : () => {}}),
            SCAN: createRouteWithGetter({     ':iouType/scan/new/:pageIndex/:reportID?'             : () => {}}),
            DISTANCE: createRouteWithGetter({ ':iouType/distance/new/:reportID?'                    : (iouType: string, reportID = '') => `${iouType}/distance/new/${reportID}`}),
        },
        AMOUNT: createRouteWithGetter({       ':iouType/amount/:transactionID/:reportID?'           : (iouType: string, transactionID: string, reportID = '') => `${iouType}/amount/${transactionID}/${reportID}`}),
        PARTICIPANTS: createRouteWithGetter({ ':iouType/participants/:transactionID/:reportID?'     : (iouType: string, transactionID: string, reportID = '') => `${iouType}/participants/${transactionID}/${reportID}`}),
        DATE: createRouteWithGetter({         ':iouType/date/:transactionID/:reportID?'             : (iouType: string, transactionID: string, reportID = '') => `${iouType}/date/${transactionID}/${reportID}`}),
        CURRENCY: createRouteWithGetter({     ':iouType/currency/:transactionID/:reportID?'         : (iouType: string, transactionID: string, reportID: string, currency: string, backTo: string) => `${iouType}/currency/${transactionID}/${reportID}?currency=${currency}&backTo=${backTo}`}),
        DESCRIPTION: createRouteWithGetter({  ':iouType/description/:transactionID/:reportID?'      : (iouType: string, transactionID: string, reportID = '') => `${iouType}/description/${transactionID}/${reportID}`}),
        CATEGORY: createRouteWithGetter({     ':iouType/category/:transactionID/:reportID?'         : (iouType: string, transactionID: string, reportID = '') => `${iouType}/category/${transactionID}/${reportID}`}),
        TAG: createRouteWithGetter({          ':iouType/tag/:transactionID/:reportID?'              : (iouType: string, transactionID: string, reportID = '') => `${iouType}/tag/${transactionID}/${reportID}`}),
        MERCHANT: createRouteWithGetter({     ':iouType/merchant/:transactionID/:reportID?'         : (iouType: string, transactionID: string, reportID = '') => `${iouType}/merchant/${transactionID}/${reportID}`}),
        WAYPOINT: createRouteWithGetter({     ':iouType/waypoint/:waypointIndex/:transactionID/:reportID?' : (iouType: string, waypointIndex: number, transactionID: string, reportID = '') => `${iouType}/waypoint/${waypointIndex}/${transactionID}/${reportID}`}),
        ADDRESS: createRouteWithGetter({      ':iouType/address/:transactionID/:reportID?'          : (iouType: string, transactionID: string, reportID = '') => `${iouType}/address/${transactionID}/${reportID}`}),
        CONFIRMATION: createRouteWithGetter({ ':iouType/confirmation/:transactionID/:reportID?'     : (iouType: string, transactionID: string, reportID = '') => `${iouType}/confirmation/${transactionID}/${reportID}`}),
    },
