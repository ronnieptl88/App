...
export default {
    ...
    MONEY_REQUEST: {
        ...
        TABS: {
            MANUAL: createRouteWithGetter({   ':iouType/manual/:reportID?'        : () => {},}),
            SCAN: createRouteWithGetter({     ':iouType/scan/:reportID?'          : () => {},}),
            DISTANCE: createRouteWithGetter({ ':iouType/distance/:reportID?'      : (iouType: string, reportID = '') => `${iouType}/${reportID}/distance`,}),
        },
        AMOUNT: createRouteWithGetter({       ':iouType/amount/:reportID?'        : (iouType: string, reportID = '') => `${iouType}/amount/${reportID}`,}),
        PARTICIPANTS: createRouteWithGetter({ ':iouType/participants/:reportID?'  : (iouType: string, reportID = '') => `${iouType}/participants/${reportID}`,}),
        DATE: createRouteWithGetter({         ':iouType/date/:reportID?'          : (iouType: string, reportID = '') => `${iouType}/date/${reportID}`,}),
        ...
        DESCRIPTION: createRouteWithGetter({  ':iouType/description/:reportID?'   : (iouType: string, reportID = '') => `${iouType}/description/${reportID}`,}),
        CATEGORY: createRouteWithGetter({     ':iouType/category/:reportID?'      : (iouType: string, reportID = '') => `${iouType}/category/${reportID}`,}),
        TAG: createRouteWithGetter({          ':iouType/tag/:reportID?'           : (iouType: string, reportID = '') => `${iouType}/tag/${reportID}`,}),
        MERCHANT: createRouteWithGetter({     ':iouType/merchant/:reportID?'      : (iouType: string, reportID = '') => `${iouType}/merchant/${reportID}`,}),
        WAYPOINT: createRouteWithGetter({     ':iouType/waypoint/:reportID?/:waypointIndex'  : (iouType: string, waypointIndex: number, reportID = '') => `${iouType}/waypoint/${reportID}/${waypointIndex}`,}),
        ADDRESS: createRouteWithGetter({      ':iouType/address/:reportID?'       : (iouType: string, reportID = '') => `${iouType}/address/${reportID}`,}),
